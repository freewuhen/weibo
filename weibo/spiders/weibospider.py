...
class WeibospiderSpider(Spider):
    ...
    def __init__(self):
        ...
        # print(self.start_list)
        # self.start_list = [('4268946481503950','test1'),('4269009338074111','test2')]
        # self.start_list = [('4270335572025636','test1')]

    # 起始爬取链接处理
    def start_requests(self):

            self.current_url = self.start_urls.format(wid=self.start_list[self.start_id_index][0],mid=self.start_list[self.start_id_index][0])
    ...
    def parse(self, response):
            try:
                weibo_id = self.start_list[self.start_id_index-1][0]
                hottop = self.start_list[self.start_id_index - 1][1]
                #将爬取的微博进行标记
                ...
                for commit in commit_group:
                    ...
                    # text = self.remove_text_dirty(text)
                    item['text'] = text
                    ...
                    homepage_id_url = self.homepage_id_url_template.format(uid=uid, luicode=self.user_luicode,value=uid, containerid=containerid)
                    # 发送请求
                ...
                if self.start_id_index > len(self.start_list)-1:
                    logging("AttributeError and self.start_list is None")
                    return
                else:
                    # 获取下条微博id
    ...
    def get_nextweibo(self):
        ...
        if self.start_id_index > len(self.start_list) - 1:# 所有的微博评论爬取完毕
            logging.info("self.start_list is None")
            return None
    ...
    def homepage_id_parse(self, response):
        ...
        try:
            ...
            #生成主页的请求链接
            homepage_url = self.homepage_url_template.format(uid=uid,luicode=self.user_luicode,value=uid,containerid=homepage_id)
